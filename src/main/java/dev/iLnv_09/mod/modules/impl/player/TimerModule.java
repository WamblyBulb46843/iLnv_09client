...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !iLnv_09.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = iLnv_09.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					iLnv_09.TIMER.set((float) Math.max(iLnv_09.TIMER.getDefault(), timer));
				} else {
					iLnv_09.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
