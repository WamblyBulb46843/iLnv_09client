...
import dev.iLnv_09.api.events.impl.ParticleEvent;
...
public class NoRender extends Module {
	...
	public final BooleanSetting noOtherSwing = add(new BooleanSetting("NoOtherSwing", true));
	public final BooleanSetting cancelExplosions = add(new BooleanSetting("CancelExplosions", false));
	public NoRender() {
	...
	public void onPacketReceive(PacketEvent.Receive event){
		...
		if (event.getPacket() instanceof EntityAnimationS2CPacket packet && noOtherSwing.getValue()) {
			if (mc.world.getEntityById(packet.getId()) instanceof PlayerEntity && packet.getAnimationId() == 0) {
				if (mc.world.getEntityById(packet.getId()) != mc.player) {
					event.setCancelled(true);
				}
			}
		}
		if (event.getPacket() instanceof ExplosionS2CPacket && cancelExplosions.getValue()) {
			event.setCancelled(true);
		}
		if (event.getPacket() instanceof EntityEquipmentUpdateS2CPacket && armor.getValue()) {
			event.setCancelled(true);
		}
		if (event.getPacket() instanceof ParticleS2CPacket packet) {
			if (explosions.getValue() && (
					packet.getParameters().getType() == ParticleTypes.EXPLOSION ||
					packet.getParameters().getType() == ParticleTypes.EXPLOSION_EMITTER
			)) {
				event.setCancelled(true);
			}
			if (fireworks.getValue() && packet.getParameters().getType() == ParticleTypes.FIREWORK) {
				event.setCancelled(true);
			}
			if (campFire.getValue() && (
					packet.getParameters().getType() == ParticleTypes.CAMPFIRE_COSY_SMOKE ||
					packet.getParameters().getType() == ParticleTypes.CAMPFIRE_SIGNAL_SMOKE
			)) {
				event.setCancelled(true);
			}
			if (effect.getValue() && (
					packet.getParameters().getType() == ParticleTypes.EFFECT ||
					packet.getParameters().getType() == ParticleTypes.ENTITY_EFFECT
			)) {
				event.setCancelled(true);
			}
		}
