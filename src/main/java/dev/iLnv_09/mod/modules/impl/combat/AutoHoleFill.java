...
public class AutoHoleFill extends Module {
    ...
    public void onUpdate() {
        ...
        CombatUtil.getEnemies(enemyRange.getValue()).stream()
                ...
                .filter(pos -> pos.toCenterPos().distanceTo(mc.player.getPos()) > selfRange.getValue() && (iLnv_09.HOLE.isHole(pos, true, true, false) || iLnv_09.HOLE.isDoubleHole(pos)))
                .distinct()
    ...
    private void tryPlaceBlock(BlockPos pos) {
        if (pos == null) return;
        if (detectMining.getValue() && iLnv_09.BREAK.isMining(pos)) return;
