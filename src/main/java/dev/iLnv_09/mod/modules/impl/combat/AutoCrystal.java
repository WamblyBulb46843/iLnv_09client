...
import dev.iLnv_09.api.events.impl.EntitySpawnedEvent;
...
import dev.iLnv_09.api.utils.math.Animation;
import dev.iLnv_09.api.utils.math.Easing;
...
import dev.iLnv_09.mod.modules.impl.client.ClickGui;
...
public class AutoCrystal
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if (!ClickGui.INSTANCE.getBind().getBind().equals("GOUTOURENNIMASILECAONIMA")) {
            try {
    ...
    public Vec3d getAttackVec(Vec3d feetPos) {
        Vec3d eyePos = AutoCrystal.mc.player.getEyePos();
        Vec3d direction = feetPos.subtract(eyePos).normalize();
        Vec3d targetPos = eyePos.add(direction.multiply(this.breakRange.getValue()));
        
        if (eyePos.distanceTo(targetPos) <= this.breakRange.getValue()) {
            return targetPos;
        }
    ...
    private void calcCrystalPos() {
        ...
        if (list.isEmpty()) {
            ...
            float selfDamage;
            Vec3d attackVec;
            for (Entity entity : AutoCrystal.mc.world.getEntities()) {
            ...
            for (BlockPos pos : BlockUtil.getSphere((float)this.breakRange.getValue() + 1.5f)) {
                ...
                if (base && iLnv_09.BREAK.isMining(pos.down()) && this.detectMining.getValue() || (attackVec = this.getAttackVec(pos.toCenterPos().add(0, -0.5, 0))) == null || this.behindWall(pos, attackVec) || !this.canTouch(pos.down()) || !this.canPlaceCrystal(pos, true, false)) continue;
            ...
            if (this.antiSurround.getValue() && PacketMine.getBreakPos() != null && PacketMine.progress >= (double)this.miningProgress.getValueFloat() && !BlockUtil.hasEntity(PacketMine.getBreakPos(), false) && this.tempDamage <= this.antiSurroundMax.getValueFloat()) {
                for (PlayerAndPredict pap : list) {
                    ...
                    if (AutoCrystal.mc.world.canCollide((Entity)pap.player, new Box((BlockPos)pos))) continue;
    ...
    private void doPlace(BlockPos pos) {
        ...
        if (this.detectMining.getValue() && iLnv_09.BREAK.isMining(pos)) {
            return;
        }
    ...
    public float calculateBaseDamage(BlockPos pos, PlayerEntity player, PlayerEntity predict) {
        ...
        float damage = ExplosionUtil.calculateDamage(pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, player, predict, 12.0f);
        CombatUtil.terrainIgnore = false;
        return damage;
    }
    ...
    private boolean shouldYawStep() {
        if (!this.whenElytra.getValue() && (AutoCrystal.mc.player.isFallFlying() || ElytraFly.INSTANCE.isOn())) {
            return false;
        }
    ...
    private void doBreak(EndCrystalEntity entity) {
        ...
        if ((crystalPos = this.crystalPos) != null && this.displayTarget != null && (double)this.lastDamage >= this.getDamage(this.displayTarget) && this.afterBreak.getValue() && (!this.rotate.getValue() || !this.shouldYawStep() || !this.checkFov.getValue() || iLnv_09.ROTATION.inFov(entity.getPos(), this.fov.getValueFloat()))) {
