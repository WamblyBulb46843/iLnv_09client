...
import dev.iLnv_09.api.events.impl.ClickBlockEvent;
...
import dev.iLnv_09.asm.accessors.IPlayerMoveC2SPacket;
...
private final BooleanSetting autoSwitch = add(new BooleanSetting("AutoSwitch", true, () -> page.is(Page.General))); // 自动切换稿子
...
private int originalSlot = -1; // 记录原始槽位
	...
	public void onUpdate() {
		...
		if (secondPos != null) {
			...
			double time = getBreakTime(secondPos, slot, 1.1);
			if (secondTimer.passed(time)) {
				...
				secondPos = null;
			} else if (autoSwitch.getValue() && originalSlot == -1 && secondTimer.passed(time * 0.9)) {
				// 挖掘即将完成，切换到稿子
				originalSlot = mc.player.getInventory().selectedSlot;
				if (slot != -1 && slot != originalSlot) {
					InventoryUtil.switchToSlot(slot);
				}
		...
		} else if (autoSwitch.getValue() && originalSlot != -1) {
			// 如果secondPos为空，确保切换回原物品
			InventoryUtil.switchToSlot(originalSlot);
			originalSlot = -1;
		}
	...
	public float getDigSpeed(BlockState state, ItemStack itemStack) {
		...
		boolean inWeb = iLnv_09.PLAYER.isInWeb(mc.player) && mc.world.getBlockState(breakPos).getBlock() == Blocks.COBWEB;
