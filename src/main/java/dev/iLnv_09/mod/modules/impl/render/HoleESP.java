...
public class HoleESP extends Module {
	...
	Type isHole(BlockPos pos) {
        if (mc.world == null) return Type.None;
		if (mc.world.isAir(pos) && (!airYCheck.getValue() || pos.getY() == mc.player.getBlockY() - 1) && iLnv_09.HOLE.isHard(pos.up())) return Type.Air;
		int blockProgress = 0;
		boolean bedRock = true;
		for (Direction i : Direction.values()) {
			if (i == Direction.UP || i == Direction.DOWN) continue;
			if (iLnv_09.HOLE.isHard(pos.offset(i))) {
		...
		if (iLnv_09.HOLE.isDoubleHole(pos)) return Type.Normal;
		return Type.None;
	}
