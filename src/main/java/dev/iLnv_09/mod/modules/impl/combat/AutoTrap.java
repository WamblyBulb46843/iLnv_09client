...
import dev.iLnv_09.mod.modules.settings.Placement;
...
public class AutoTrap
    ...
    private void trapTarget(PlayerEntity target) {
        if (onlyHole.getValue() && !iLnv_09.HOLE.isHole(EntityUtil.getEntityPos(target))) return;
    ...
    private void doTrap(BlockPos pos) {
        ...
        if (antiStep.getValue() && (iLnv_09.BREAK.isMining(pos.up(2)) || !onlyBreak.getValue())) {
        ...
        if (chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (!onlyBreaking.getValue() || iLnv_09.BREAK.isMining(pos.up(2))) {
                    tryPlaceObsidian(offsetPos);
    ...
    public BlockPos getHelper(BlockPos pos) {
        ...
        for (Direction i : Direction.values()) {
            if (checkMine.getValue() && iLnv_09.BREAK.isMining(pos.offset(i))) continue;
    ...
    private void tryPlaceBlock(BlockPos pos, boolean anchor, boolean sand, boolean web) {
        ...
        if (iLnv_09.BREAK.isMining(pos)) return;
