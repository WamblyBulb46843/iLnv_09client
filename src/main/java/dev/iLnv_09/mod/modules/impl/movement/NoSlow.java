...
public class NoSlow extends Module {
    ...
    private final BooleanSetting potions = add(new BooleanSetting("Potions", true));
    ...
    public void onUpdate(UpdateWalkingPlayerEvent event) {
        ...
        if (mc.player.isUsingItem() && !mc.player.isRiding() && !mc.player.isFallFlying()) {
            switch (mode.getValue()) {
                case NCP -> {
                    if (potions.getValue() && mc.player.hasStatusEffect(StatusEffects.SLOWNESS)) {
                        mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, mc.player.getBlockPos(), Direction.DOWN));
                    }
