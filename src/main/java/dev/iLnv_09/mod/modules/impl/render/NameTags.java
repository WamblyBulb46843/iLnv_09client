...
import dev.iLnv_09.mod.modules.impl.player.Freecam;
...
public class NameTags extends Module {
    ...
    private final BooleanSetting chickens = add(new BooleanSetting("Chickens", false));
    ...
    public void onRender2D(DrawContext context, float tickDelta) {
        for (Entity ent : mc.world.getEntities()) {
            ...
            boolean isChicken = chickens.getValue() && ent instanceof ChickenEntity;

            if (!isPlayer && !isChicken) {
                continue;
            }
            ...
            if (vector.z > 0 && vector.z < 1) {
                ...
                if (isPlayer) {
                    ...
                    if (god.getValue() && player.hasStatusEffect(StatusEffects.SLOWNESS)) {
                        final_string += "§4GOD ";
                    }
                    if (ping.getValue()) {
                        final_string += getEntityPing(player) + "ms ";
                    }
                    if (gamemode.getValue()) {
                        final_string += translateGamemode(getEntityGamemode(player)) + " ";
                    }
                }
                ...
                if (isPlayer) {
                    PlayerEntity player = (PlayerEntity) ent;
                    if (health.getValue()) {
                        final_string += " " + getHealthColor(player) + round2(player.getAbsorptionAmount() + player.getHealth());
                    }
                }
                ...
                if (isPlayer) {
                    PlayerEntity player = (PlayerEntity) ent;
                    if (pops.getValue() && iLnv_09.POP.getPop(player.getName().getString()) != 0) {
                        final_string += " §bPop" + " " + Formatting.LIGHT_PURPLE + iLnv_09.POP.getPop(player.getName().getString());
                    }
                }
                ...
                if (font.getValue() == Font.Fancy) {
                    FontRenderers.ui.drawString(context.getMatrices(), final_string, tagX, (float) posY - 10, isPlayer && iLnv_09.FRIEND.isFriend((PlayerEntity) ent) ? friendColor.getValue().getRGB() : this.color.getValue().getRGB());
                } else {
                    context.getMatrices().push();
                    context.getMatrices().translate(tagX, ((float) posY - 11), 0);
                    context.drawText(mc.textRenderer, final_string, 0, 0, isPlayer && iLnv_09.FRIEND.isFriend((PlayerEntity) ent) ? friendColor.getValue().getRGB() : this.color.getValue().getRGB(), true);
                    context.getMatrices().pop();
                }
                context.getMatrices().pop();
            }
