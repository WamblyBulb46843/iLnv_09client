...
import dev.iLnv_09.mod.modules.impl.client.Notify;
...
public final class PopChams extends Module {
    ...
    private void onTotemPop(TotemEvent e) {
        ...
        if (Notify.INSTANCE.pop.getValue() && mc.player.distanceTo(e.getPlayer()) <= Notify.INSTANCE.targetRange.getValue()) {
            sendNotify("§4[!]§f " + e.getPlayer().getName().getString() + " 被击破一个图腾！");
        }
