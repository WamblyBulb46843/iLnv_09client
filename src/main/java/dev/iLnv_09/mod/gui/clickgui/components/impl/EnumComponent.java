package dev.iLnv_09.mod.gui.clickgui.components.impl;
...
import dev.iLnv_09.core.impl.GuiManager;
import dev.iLnv_09.api.utils.render.Render2DUtil;
import dev.iLnv_09.api.utils.render.TextUtil;
import dev.iLnv_09.mod.gui.clickgui.components.Component;
import dev.iLnv_09.mod.gui.clickgui.ClickGuiScreen;
import dev.iLnv_09.mod.gui.clickgui.tabs.ClickGuiTab;
...
public class EnumComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (ClickGui.INSTANCE.mainEnd.booleanValue) {
			Render2DUtil.drawRectHorizontal(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : iLnv_09.GUI.getColor(), ClickGui.INSTANCE.mainEnd.getValue());
		} else {
			Render2DUtil.drawRect(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : iLnv_09.GUI.getColor());
		}
