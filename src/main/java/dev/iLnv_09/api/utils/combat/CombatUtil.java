...
public class CombatUtil implements Wrapper {
    ...
    public static PlayerEntity getNearestEnemy(double range) {
        PlayerEntity nearest = null;
        double nearestDistance = range;

        for (PlayerEntity player : mc.world.getPlayers()) {
            if (player == null || player == mc.player || player.isDead() || !isEnemy(player))
                continue;
    ...
    public static boolean isEnemy(PlayerEntity player) {
        // 这里根据你的敌人判断逻辑来实现
        // 可能是队伍检查、朋友列表检查等
        return !player.getUuid().equals(mc.player.getUuid()) && player.isAlive();
    }
    public static boolean isHard(BlockPosX blockPosX) {
        return false;
    }
    ...
    public static void attackCrystal(Entity crystal, boolean rotate, boolean usingPause) {
        ...
        if (crystal != null) {
            ...
            if (rotate && AntiCheat.INSTANCE.attackRotate.getValue()) iLnv_09.ROTATION.lookAt(new Vec3d(crystal.getX(), crystal.getY() + 0.25, crystal.getZ()));
    ...
    public static boolean isValid(Entity entity, double range) {
        boolean invalid = entity == null || !entity.isAlive() || entity.equals(mc.player) || entity instanceof PlayerEntity player && iLnv_09.FRIEND.isFriend(player) || mc.player.getPos().distanceTo(entity.getPos()) > range;
    ...
    public static boolean isSurrounded(PlayerEntity player) {
        ...
        if (!mc.world.getBlockState(playerPos.down()).isSolid()) return false;
