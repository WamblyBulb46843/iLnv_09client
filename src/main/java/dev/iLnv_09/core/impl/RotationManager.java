...
import dev.iLnv_09.api.events.impl.*;
...
import dev.iLnv_09.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), iLnv_09.ROTATION.rotationYaw, iLnv_09.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], iLnv_09.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - iLnv_09.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public boolean isRotating() {
        return directionVec != null && !ROTATE_TIMER.passed((long) (AntiCheat.INSTANCE.rotateTime.getValue() * 1000));
    }
    ...
    private void updateNext() {
        ...
        iLnv_09.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        iLnv_09.EVENT_BUS.post(lookAtEvent);
